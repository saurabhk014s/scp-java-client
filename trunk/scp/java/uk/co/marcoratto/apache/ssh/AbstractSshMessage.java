...
public abstract class AbstractSshMessage {
	...
	private static Logger logger = Logger.getLogger(AbstractSshMessage.class);
    ...
    /**
     * Reads the response, throws a ScpException if the response
     * indicates an error.
     * @param in the input stream to use
     * @throws IOException on I/O error
     * @throws SCPPException on other errors
     */
    protected void waitForAck(InputStream in)
        throws IOException, SCPPException {
        int b = in.read();
        ...
        if (b == -1) {
            ...
            throw new SCPPException("No response from server");
        } else if (b != 0) {
            ...
            if (b == 1) {
                throw new SCPPException("server indicated an error: "
                                         + sb.toString());
            } else if (b == 2) {
                throw new SCPPException("server indicated a fatal error: "
                                         + sb.toString());
            } else {
                throw new SCPPException("unknown response, code " + b
                                         + " message: " + sb.toString());
            }
        }
    }
     ...
     */
    public abstract void execute() throws IOException, JSchException, SCPPException;
    ...
    protected void logStats(long timeStarted,
        ...
        logger.info("File transfer time: " + format.format(duration)
    ...
    public SCPListener getScpLogger() {
		return scpLogger;
	}

	public void setScpLogger(SCPListener scpLogger) {
		this.scpLogger = scpLogger;
	}
	...
	public void setChmodDirectory(String s) {
		this.chmodDirectory = s;
	}
